...
st.set_page_config(
    page_title="Tarántula Taller", page_icon="🕷️", layout="centered"
)
...
# --- 2. INICIALIZACIÓN DE BASE DE DATOS Y MEMORIA ---
database.init_db()
...
if not st.session_state["user_id"]:
  ...
  st.write(
      "Laboratorio de fotografía analógica. Revelado, digitalización y venta."
  )
    ...
    with st.form("registro_form"):
      ...
      if st.form_submit_button("Registrarse"):
        if reg_nombre and reg_email and reg_password:
          if database.registrar_usuario(
              reg_nombre, reg_email, reg_password, reg_telefono
          ):
...
# --- 4. APLICACIÓN PRINCIPAL ---
  ...
  # ---- VISTA DE ADMINISTRADOR (EL TALLER) ----
  if st.session_state["rol"] == "admin":
    st.subheader("Panel de Control (Admin)")

    tab_admin1, tab_admin2 = st.tabs(["Gestión de Solicitudes", "Inventario"])
    ...
    with tab_admin1:
      ...
      if not pedidos_df.empty:
        ...
        st.markdown("---")
        st.write("### Actualizar Estado y Subir Archivos")
        with st.form("update_form"):
          ...
          p_link = st.text_input("Enlace de Descarga (Drive / S3 / Supabase)")
          if st.form_submit_button("Actualizar Solicitud"):
            database.actualizar_pedido(p_id, p_est, p_link)
            st.success(f"Solicitud #{p_id} actualizada.")
            st.rerun()
      else:
        st.info("No hay solicitudes pendientes en el sistema.")
  ...
  # ---- VISTA DE CLIENTE ----
    ...
    total_items = sum(
        item["cantidad"] for item in st.session_state["carrito"].values()
    )

    tab1, tab2, tab3, tab4 = st.tabs([
        "Tienda",
        "Revelado",
        f"Mi Carrito ({total_items})",
        "Bóveda Digital",
    ])
    ...
    with tab1:
      st.markdown(
          '<img'
          ' src="https://images.unsplash.com/photo-1549264875-e854ba0d10b7?w=800&q=80"'
          ' style="width: 100%; margin-bottom: 20px;" alt="Banner">',
          unsafe_allow_html=True,
      )
      st.markdown("### Films")
      st.markdown(
          "<p style='font-style: italic;'>Our most popular</p>",
          unsafe_allow_html=True,
      )
      ...
      for index, row in cat_df.iterrows():
        ...
        with c2:
          st.markdown(
              ...
              f" font-size:16px;'>{row['nombre_producto']}</span>",
              unsafe_allow_html=True,
          )
          st.markdown(
              f"<span style='color:#666; font-size:14px;'>$ {row['precio']:,.0f}"
              " COP</span>",
              unsafe_allow_html=True,
          )
        with c3:
    ...
    # PESTAÑA 2: SOLICITUD DE REVELADO (OPCIONES SIMPLIFICADAS)
    with tab2:
      st.markdown("### Develop Your Film")
      with st.form("form_revelado"):
        st.write("Completa la solicitud para traer tus rollos al taller.")
        tipo = st.selectbox(
            "Proceso Químico",
            ["A color", "Blanco y Negro"],
        )
        qty = st.number_input("Cantidad de rollos", min_value=1, value=1)
        obs = st.text_area("Instrucciones o notas especiales")

        if st.form_submit_button("Enviar Solicitud de Revelado"):
          database.guardar_pedido(
              st.session_state["user_id"],
              f"Servicio de Revelado ({tipo})",
              qty,
              "Revelado en Lab",
              obs,
          )
          st.success(
              "¡Tu solicitud fue enviada con éxito! Revisa su estado en la"
              " Bóveda Digital."
          )

    # PESTAÑA 3: CARRITO DE COMPRAS Y REDIRECCIÓN A WHATSAPP
    with tab3:
      ...
      if not st.session_state["carrito"]:
        ...
        for prod_id, datos in st.session_state["carrito"].items():
          precio_unitario = cat_df.loc[
              cat_df["id"] == prod_id, "precio"
          ].values[0]
        ...
        st.markdown("---")
        st.markdown(
            f"<h3 style='text-align: right;'>Total: $ {total_pagar:,.0f}"
            " COP</h3>",
            unsafe_allow_html=True,
        )
        ...
        # numero del taller
        numero_taller = "573005431546"
        link_wpp = f"https://wa.me/{numero_taller}?text={mensaje_codificado}"

        if st.button("Confirmar Pedido y Enviar por WhatsApp"):
          ...
          for prod_id, datos in st.session_state["carrito"].items():
            database.guardar_pedido(
                ...
                datos["nombre"],
                datos["cantidad"],
                "Compra de Rollo (Tienda)",
                "Pedido desde Carrito Web",
            )
          ...
          st.markdown(
              f'<meta http-equiv="refresh" content="0; url={link_wpp}">',
              unsafe_allow_html=True,
          )
          
          # Mensaje y botón de respaldo por si el navegador bloquea la auto-redirección
          st.success("¡Pedido registrado exitosamente!")
          st.markdown(
              f'<a href="{link_wpp}" target="_blank" style="display:inline-block;'
              ' margin-top:10px; padding:12px 24px; background-color:#000;'
              ' color:#fff; text-decoration:none; font-family:\'Arimo\','
              ' sans-serif; font-weight:bold; font-style:italic;'
              ' text-transform:uppercase;">📲 Abrir WhatsApp</a>',
              unsafe_allow_html=True,
          )

    # PESTAÑA 4: BÓVEDA DIGITAL
    with tab4:
      st.markdown("### Tus Solicitudes y Negativos Digitales")
      mis_pedidos = database.obtener_pedidos_usuario(
          st.session_state["user_id"]
      )
      if mis_pedidos.empty:
        st.info("Aún no tienes solicitudes registradas en la bóveda.")
      else:
        for _, row in mis_pedidos.iterrows():
          with st.expander(
              f"Solicitud #{row['id']} - {row['item_solicitado']}"
          ):
            st.write(f"**Fecha:** {row['fecha']}")
            st.write(f"**Estado actual:** {row['estado']}")
            st.write(f"**Tipo:** {row['tipo_servicio']}")

            if row["link_descarga"] and row["link_descarga"].strip() != "":
              st.success("🎉 ¡Archivos listos para descargar!")
              st.markdown(
                  "[📥 Descargar Archivos (Alta"
                  f" Resolución)]({row['link_descarga']})"
              )
            else:
              st.warning(
                  "Tus rollos/pedido se encuentran en proceso en el taller."
              )
